...
title('GFPmut3-sRNA Response -- Predation Resource Competition', 'Interpreter','latex');
...
function dy = sRNA_ODEs(~, y, k, F, sRNA_trans, GFPmut3_trans, mu)
    ...
    term  = (k(1)/k(4))*y(1)*y(4)*k(2)/denom;
    
    
    dy = zeros(5,1);

    dy(1) = G - (mu + F(1))*y(1) - term;
    dy(2) = k(31)*y(1) - (mu + F(2))*y(2);
    dy(3) = F(2)*y(2) - mu * y(3);
    dy(4) = sRNA_t  - (mu + F(3))*y(4) - term + k(18)*k(17)*y(5);
    dy(5) = term - (mu + k(17))*y(5);
end